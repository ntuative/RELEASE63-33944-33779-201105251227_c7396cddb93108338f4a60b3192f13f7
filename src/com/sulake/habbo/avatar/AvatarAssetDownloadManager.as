package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private var var_1159:Dictionary;
      ...
      private var var_938:int = 3;
      
      private var var_418:AssetLoaderStruct;
      
      private var var_937:Timer;
      ...
      private const const_2220:int = 100;
      
      private const const_1906:int = 2;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_937 = new Timer(this.const_2220,1);
         this.var_937.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_418)
         {
            ...
            this.var_418.addEventListener(AssetLoaderEvent.const_46,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_418)
         {
            ...
            this.var_418.removeEventListener(AssetLoaderEvent.const_46,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_938);
            if(this.var_1450.indexOf("?") > 0)
            {
               _loc2_ = this.var_1450 + "&retry=" + this.var_938;
            }
            else
            {
               _loc2_ = this.var_1450 + "?retry=" + this.var_938;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         {
            _loc3_ = new AvatarAssetDownloadLibrary(_loc2_.@id,_loc2_.@revision,this.var_2416,this._assets);
            _loc3_.addEventListener(Event.COMPLETE,this.libraryComplete);
            for each(_loc4_ in _loc2_.part)
            {
               _loc5_ = _loc4_.@type + ":" + _loc4_.@id;
               _loc6_ = this.var_1159[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1159[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function isReady(param1:IAvatarFigureContainer) : Boolean
      {
         ...
         var _loc2_:Array = this.method_6(param1);
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(!this.var_1770)
         {
            this.var_1160.push([param1,param2]);
            return;
         }
         var _loc3_:String = param1.getFigureString();
         var _loc4_:Array = this.method_6(param1);
         if(_loc4_.length > 0)
         {
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         {
            delete this.var_936[_loc3_];
         }
         while(_loc6_ < this.var_747.length)
         {
            _loc5_ = this.var_747[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_747.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_937.start();
      ...
      private function method_6(param1:IAvatarFigureContainer) : Array
      {
         var _loc5_:* = null;
         var _loc6_:* = null;
         var _loc7_:int = 0;
      ...
      {
         var _loc1_:* = null;
         while(this.var_746.length > 0 && this.var_747.length < this.const_1906)
         {
            _loc1_ = this.var_746[0];
            _loc1_.startDownloading();
            this.var_747.push(this.var_746.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_746.indexOf(param1) == -1 && this.var_747.indexOf(param1) == -1)
         {
            this.var_746.push(param1);
            this.processPending();
         }
      }
