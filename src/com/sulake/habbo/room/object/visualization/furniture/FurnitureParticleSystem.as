package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_273:IRoomObjectSprite;
      ...
      private var var_1257:Number = 1.0;
      
      private var var_645:BitmapData;
      
      private var var_823:ColorTransform;
      
      private var var_1543:ColorTransform;
      
      private var var_1946:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_823.alphaMultiplier = 1;
         this.var_1543 = new ColorTransform();
         this.var_1946 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_105 = this.var_1258[param1];
         this.var_1013 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1945 >= 0)
         {
            this.var_273 = this._visualization.getSprite(this.var_1945);
            if(this.var_273 && this.var_273.asset)
            {
               if(this.var_273.width <= 1 || this.var_273.height <= 1)
               {
                  return;
               }
               if(this.var_90 == null)
               {
                  this.var_90 = this.var_273.asset.clone();
                  if(this.var_823.alphaMultiplier != 1)
                  {
                     this.var_645 = new BitmapData(this.var_90.width,this.var_90.height,true,4278190080);
                  }
               }
               this.var_2385 = -this.var_273.offsetX;
               this.var_2386 = -this.var_273.offsetY;
               this.var_273.asset = this.var_90;
            }
            if(this.var_90)
            {
               this.var_90.fillRect(this.var_90.rect,4278190080);
            }
            if(this.var_645)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_105.y * this.var_1257;
         }
         return 0;
      }
      ...
      {
         if(!this.var_105 || !this.var_273)
         {
            return;
         }
         if(this.var_90 && this.var_273.asset != this.var_90)
         {
            this.var_273.asset = this.var_90;
         }
         if(this.var_1013)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1013 && this.var_105.hasIgnited)
         {
            this.var_1013 = true;
         }
         _loc8_ = this.var_1267 * this.var_1257;
         this.var_105.update();
         if(this.var_1013)
         {
            if(this.var_105.roomObjectSpriteId >= 0)
            ...
            {
               this.var_90.draw(this.var_645,this.var_1946,this.var_823,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_105.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2385 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1257;
               _loc4_ = this.var_2386 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1257;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1543.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_90.draw(_loc7_,this._translationMatrix,this.var_1543,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
                     _loc10_ = new Point(_loc3_ + _loc6_.offsetX,_loc4_ + _loc6_.offsetY);
                     this.var_90.copyPixels(_loc7_,_loc7_.rect,_loc10_,null,null,true);
                  }
               }
               else
               {
                  _loc5_ = new Rectangle(_loc3_ - 1,_loc4_ - 1,2,2);
                  this.var_90.fillRect(_loc5_,4294967295);
               }
            }
            this.var_90.unlock();
         }
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2704 = parseInt(param1.@size);
         this.var_1945 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1267 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1257 = this.var_2704 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_823.alphaMultiplier = this._blend;
