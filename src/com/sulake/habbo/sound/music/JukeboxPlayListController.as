package com.sulake.habbo.sound.music
{
   ...
   public class JukeboxPlayListController implements IPlayListController
   {
      ...
      private function onNowPlayingMessage(param1:IMessageEvent) : void
      {
         ...
         {
            this.var_228.playSong(_loc3_.currentSongId,HabboMusicPrioritiesEnum.const_132,_loc3_.syncCount / 1000,0,1,1);
            this.var_422 = _loc3_.currentSongId;
         }
         else
         {
            this.stopPlaying();
         ...
         this._soundManager.events.dispatchEvent(new NowPlayingEvent(NowPlayingEvent.const_99,HabboMusicPrioritiesEnum.const_132,_loc3_.currentSongId,_loc3_.currentPosition));
      }
      ...
      private function onJukeboxSongDisksMessage(param1:IMessageEvent) : void
      {
         ...
         while(_loc4_ < _loc3_.songDisks.length)
         {
            ...
            if(_loc7_ == null)
            {
               ...
               if(this.var_752.indexOf(_loc5_) < 0)
               {
                  this.var_752.push(_loc5_);
                  this.var_228.requestSongInfoWithoutSamples(_loc5_);
               }
            }
            _loc7_.diskId = _loc6_;
            this.var_503.push(_loc7_);
            _loc4_++;
         }
         ...
         while(_loc2_ < this.length)
         {
            ...
            if(_loc4_.id == param1.id)
            {
               ...
               if(_loc6_ != null)
               {
                  ...
                  this.var_503[_loc2_] = _loc6_;
               }
               break;
            }
            _loc2_++;
         }
         var _loc3_:int = this.var_752.indexOf(param1.id);
         if(_loc3_ >= 0)
         {
            this.var_752.splice(_loc3_,1);
         }
         if(this.var_752.length == 0)
         {
            this._events.dispatchEvent(new PlayListStatusEvent(PlayListStatusEvent.const_101));
