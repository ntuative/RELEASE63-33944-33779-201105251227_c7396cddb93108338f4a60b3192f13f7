package com.sulake.habbo.messenger.domain
{
   ...
   public class Conversations
   {
      ...
      private static var var_1172:int = 1;
       
      
      private var var_423:IConversationsDeps;
      ...
      public function setSelected(param1:Conversation) : void
      {
         ...
         var _loc2_:int = this.var_139.indexOf(param1);
         while(this._startIndex + this.var_423.getTabCount() <= _loc2_)
         {
            ++this._startIndex;
         }
      }
      ...
      public function closeConversation() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc2_ = this.var_139.indexOf(_loc1_);
            Util.remove(this.var_139,_loc1_);
            ...
            while(_loc2_ >= 0)
            {
               ...
               {
                  this.var_139[_loc2_].setSelected(true);
                  break;
               }
               _loc2_--;
            }
      ...
      {
         this.addConversation(var_1172++);
      }
      
      public function addConversation(param1:int) : Conversation
      {
         var _loc2_:Boolean = this._closedConversations.length == 0 && this.var_139.length == 0;
         var _loc3_:Conversation = this.addConversationInt(param1);
         if(_loc3_ == null)
         {
            return null;
         }
         if(this.var_139.length == 1)
         {
            this.var_139[0].setSelected(true);
         }
         if(_loc2_)
         {
            _loc3_.addMessage(new Message(Message.const_703,0,this.var_423.getText("messenger.moderationinfo"),""));
         }
         return _loc3_;
      }
      ...
      {
         var _loc2_:int = this.var_139.length;
         ...
         this.var_423.addMsgToView(_loc3_,param1);
         if(_loc2_ != this.var_139.length || _loc4_ != _loc3_.newMessageArrived)
         {
            this.var_423.refresh(false);
         }
      }
      ...
      public function setOnlineStatusAndUpdateView(param1:int, param2:Boolean) : void
      {
         ...
         this.var_423.addMsgToView(_loc3_,_loc4_);
      }
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_139)
         {
            if(_loc1_.selected)
            {
               return _loc1_;
      ...
      private function addConversationInt(param1:int) : Conversation
      {
         ...
         if(_loc2_ != null)
         {
            ...
            this.var_139.push(_loc2_);
            return _loc2_;
         }
         _loc2_ = this.var_423.createConversation(param1);
         if(_loc2_ == null)
         {
            return null;
         }
      ...
      {
         this._startIndex = Math.min(this._startIndex,this.var_139.length - this.var_423.getTabCount());
         this._startIndex = Math.max(0,this._startIndex);
      }
      
      private function getOnlineInfoMsg(param1:Boolean) : Message
      {
         return new Message(Message.const_745,0,this.getOnlineText(param1),Util.getFormattedNow());
      }
      
      private function getOnlineText(param1:Boolean) : String
      {
         return this.var_423.getText(!!param1 ? "messenger.notification.online" : "messenger.notification.offline");
      }
   }
}
