package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1449:String = "service-libraries";
      
      private static const const_1446:String = "library";
      
      private static const const_1443:String = "component-libraries";
      
      private static const const_1444:String = "library";
      
      private static const const_1442:String = "error_data";
      
      private static const const_1447:String = "error_cat";
      
      private static const const_1445:String = "error_desc";
       
      
      private var var_365:LibraryLoaderQueue;
      
      private var var_772:IEventDispatcher;
      
      private var var_506:uint;
      
      private var var_364:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         switch(param2 & 0)
         {
            case Core.const_1159:
               ...
               this.var_364 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_944:
               debug("Core; using complex frame update handler");
               this.var_364 = this.complexFrameUpdateHandler;
               break;
            case Core.const_954:
               debug("Core; using profiler frame update handler");
               this.var_364 = this.profilerFrameUpdateHandler;
               this.var_204 = new Profiler(this);
               attachComponent(this.var_204,[new IIDProfiler()]);
               var_405.addChild(new ProfilerViewer(this.var_204));
               break;
            case Core.const_618:
               debug("Core; using experimental frame update handler");
               this.var_364 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_364 = this.debugFrameUpdateHandler;
         }
      }
      
      public function getNumberOfFilesPending() : uint
      {
         return this.var_365.length;
      }
      
      public function getNumberOfFilesLoaded() : uint
      {
         return this.var_506 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_261[i] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1442,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1447,String(param3));
         ErrorReportStorage.setParameter(const_1445,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_772 = loadingEventDelegate;
         if(this.var_365 == null)
         {
            this.var_365 = new LibraryLoaderQueue(_debug);
         }
         try
         {
         ...
         try
         {
            node = config.child(const_1449)[0];
            if(node != null)
            {
               list = node.child(const_1446);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1443)[0];
            if(node != null)
            {
               list = node.child(const_1444);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function errorInLoadingProcess(param1:LibraryLoaderEvent = null) : void
      {
         ...
         this.error("Failed to download library \"" + _loc2_.url + "\" HTTP status " + param1.status + ": " + _loc2_.getLastErrorMessage(),true,Core.const_1254);
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_772)
         {
            this.var_772.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_772)
         {
            ...
            this.var_772.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_772)
                  {
                     this.var_772.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_506 - this.var_365.length,this.var_506,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_204 ? 0 : this.var_1466 & 0;
         if(_loc3_ == Core.const_618)
         {
            this.var_261[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_261[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_204 ? 0 : this.var_1466 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_104)
         {
            _loc3_ = this.var_261[_loc5_] as Array;
            if(_loc4_ == Core.const_618)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_364(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_613[priority] = 0;
            receivers = this.var_261[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         while(priority < CoreComponentContext.const_104)
         {
            ...
            {
               if(this.var_613[priority] < priority)
               {
                  ++this.var_613[priority];
                  skip = true;
               }
            }
            if(!skip)
