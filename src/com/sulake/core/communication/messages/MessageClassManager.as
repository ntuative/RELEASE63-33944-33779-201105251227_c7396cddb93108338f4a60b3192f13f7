package com.sulake.core.communication.messages
{
   ...
   public class MessageClassManager implements IMessageClassManager
   {
      ...
      private function registerMessageEvent(param1:int, param2:Class) : Boolean
      {
         ...
         {
            _loc5_ = _loc3_.@name;
            if(this.var_741[param1] == null)
            {
               this.var_741[param1] = [param2];
            }
            else
            {
               _loc6_ = this.var_741[param1];
               _loc6_.push(param2);
            }
            return true;
         }
      ...
      public function getMessageComposerID(param1:IMessageComposer) : int
      {
         ...
         {
            _loc4_ = this.var_928[_loc3_] as Class;
            if(param1 is _loc4_)
            {
      ...
      {
         var _loc2_:Array = this.var_741[param1];
         if(_loc2_ != null)
         {
            return _loc2_;
         }
      ...
      public function toString() : String
      {
         ...
         for(_loc2_ in this.var_928)
         {
            _loc1_ += _loc2_ + " -> " + this.var_928[_loc2_] + "\n";
         }
         _loc1_ += "Registered Message Event Classes: \n";
         for(_loc2_ in this.var_741)
         {
            _loc1_ += _loc2_ + " -> " + this.var_741[_loc2_] + "\n";
         }
         return _loc1_;
      }
   }
